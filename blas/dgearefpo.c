...
for(int k=0; k<maxiter; k++)
    ...
    //k=k+1;
    //C22=A22-X0*A12;
    dlacpy_("A",&pn,&pn,A22,&pn,C22,&pn);
    dgemm_("N","N",&pn,&pn,&pn,&mone,X0,&pn,A12,&pn,&done,C22,&pn);
    #if DEBUG
    fprintf(stdout,"k=%d\n",k);
    fprintf(stdout,"C22=\n");
    for (i=0;i<pn;i++)
    ...
    #endif
    //C21=-A21+X0*A11;
    dlacpy_("A",&pn,&pn,A21,&pn,C21,&pn);
    dgemm_("N","N",&pn,&pn,&pn,&done,X0,&pn,A11,&pn,&mone,C21,&pn);
    #if DEBUG
    fprintf(stdout,"k=%d\n",k);
    fprintf(stdout,"C21=\n");
    for (i=0;i<pn;i++)
    ...
    //C21=C22\C21;
    dgesv_(&pn,&pn,C22,&pn,ipiv,C21,&pn,&info);
    #if DEBUG
    fprintf(stdout,"C21=\n");
    for (i=0;i<pn;i++)
        {
    ...
    fprintf(stdout,"\n");
    #endif
    norma=dlange_("F",&pn,&pn,X0,&pn,WORK);
    #if DEBUG
    fprintf(stdout,"norma=%lf\n",norma);
    #endif
    //X0=C21;
    dlacpy_("A",&pn,&pn,C21,&pn,X0,&pn);
    #if DEBUG
